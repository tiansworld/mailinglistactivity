...
readurl <- function(url) {
    htmlcode <- getURL(url)
    htmltable <- xpathApply(htmlTreeParse(transhtml,useInternalNodes = T),
    "//td", function(x) xmlValue(x))
}

## Filter the list according to the requirment
filter <- function(htmltable,monthstart=4,yearstart=monthstart+2,by=3,end) {
    #Add a judgment, if end exists or not
#if end exists,
    fltmonth <- seq(monthstart,end,by)
    fltsize <- seq(yearstart,end,by)
#if end doesn't exists, use length of htmltable instead
    fltmonth <- seq(monthstart,length(htmltable),by)
    fltmonth <- seq(yearstart,length(htmltable),by)
}
...
##         geom_line(size=1) + labs(title="Activity of translation mailing list",x = "Month",y = "Archive Size (KB)")
